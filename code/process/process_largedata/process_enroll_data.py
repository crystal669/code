...
def process_not_timeinfo(data):
    ...
    print(data.shape[0])
    data=data.drop(data[(data['action']=='create_thread') | (data['action']=='delete_thread')].index)
    print(data.shape[0])
    ...
    action_dict={'click_about':'navigate','click_info':'navigate',
                 ...
                 'click_forum':'forum','create_comment':'forum','delete_comment':'forum','close_forum':'forum',
...
def process_timeinfo(data):
    ...
    for i,row in data.iterrows(): 
        if (new_data.empty) or (row.session_id!=new_data.loc[last_index].session_id):
            print('empty!')
            ...
            if row.action!=new_data.loc[last_index].action:
...
save_dir="F:\\DeepLearning\\DataSet\\mydataset\\process\\kddcup\\log_action_time"
...
train_data=pd.read_csv(read_path1,usecols=['enroll_id', 'username','course_id','session_id','action','time'])
test_data=pd.read_csv(read_path2,usecols=['enroll_id', 'username','course_id','session_id','action','time'])
